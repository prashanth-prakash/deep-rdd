...
model_out = './results/spiking_just_rdd_M_%d_N_%d_batchsize_%d.pkl'%(M, N, batch_size)
results_out = './results/spiking_just_rdd_M_%d_N_%d_batchsize_%d_results.pkl'%(M, N, batch_size)
...
for i in range(M):
    ...
    for j in range(N):
        print("Iteration: %d"%j)
        train_loss, train_acc, metrics = lif.train_just_RDD(deltaT)
